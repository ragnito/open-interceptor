...
pub async fn forward(
    ...
    // Always sanitize the body before forwarding to Anthropic:
    // 1. Rewrite model if remap applies.
    // 2. Strip Thinking/RedactedThinking blocks from messages.
    //    These blocks may have been generated by our OpenAI→Anthropic
    //    translation layer (from DeepSeek's reasoning_content) and carry
    //    fake signatures that Anthropic will reject.
...
pub fn sanitize_body(body: &Bytes, effective_model: &str, request_model: &str) -> Result<Bytes, ForwardError> {
...
pub fn strip_thinking_blocks(content: &mut serde_json::Value) {
    if let Some(blocks) = content.as_array_mut() {
        ...
        if removed > 0 {
            tracing::debug!(removed, "stripped Thinking blocks before forwarding to Anthropic");
        }
    }
}
