...
- [ ] **T0.4** Configurar `Cargo.toml` con dependencias core: `tokio` (full), `axum` 0.7, `hyper` 1, `reqwest` 0.12 (rustls-tls, stream, json), `serde` (derive), `serde_json`, `serde_yaml`, `clap` v4 (derive), `tracing`, `tracing-subscriber`, `tracing-appender`, `globset`, `shellexpand`, `anyhow`, `thiserror`, `eventsource-stream`, `futures`, `tokio-util`
  - Definir `[profile.release]` con `lto = "fat"`, `codegen-units = 1`, `strip = true` para binario optimizado.
